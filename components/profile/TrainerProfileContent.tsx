...
export function TrainerProfileContent(): ReactElement {
  const {
    aiCreditsTotal,
    aiCreditsUsed,
    mockLifecycle,
  ...
  const authLabel = mockLifecycle.mockAuthStatus === "authenticated_demo" ? "демо" : "не выполнен";
  ...
  return (
    <main className="flex w-full min-w-0 flex-col gap-5 px-4 py-6 pb-[calc(6rem+max(0.35rem,env(safe-area-inset-bottom,0px)))]">
      <header className="flex flex-col gap-1">
        ...
        <p className="text-xs text-[var(--tg-muted)]">Демо-режим: данные только в этой сессии браузера.</p>
      </header>
      ...
      <section className={sectionCard} aria-labelledby="profile-account">
        ...
        <dl className="mt-3 flex flex-col gap-2 text-sm">
          ...
          <div className="flex justify-between gap-3">
            ...
            <dd className="min-w-0 text-right text-[var(--text-secondary)]">будет подключён позже</dd>
          </div>
        </dl>
        <Link
          href="/profile/setup"
      ...
      <section className={sectionCard} aria-labelledby="profile-access">
        ...
        <p className="mt-3 text-sm font-medium text-[var(--text-primary)]">
          Текущий режим: Демо · {accessLabel}
        </p>
      ...
      <section className={sectionCard} aria-labelledby="profile-ai">
        ...
        <p className="mt-2 text-sm leading-relaxed text-[var(--text-secondary)]">
          AI-подготовки помогают сформулировать короткую подсказку перед тренировкой или сообщение клиенту.
        </p>
        <p className="mt-3 text-sm text-[var(--tg-muted)]">
          ...
          AI-подготовок в демо, использовано {aiCreditsUsed}.
        </p>
